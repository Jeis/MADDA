"""
Spatial Enterprise Observability - Span Processors
...
def create_span_processors(
    ...
    try:
        # OTLP exporter (primary) - enterprise production configuration
        # Handle compression parameter compatibility across OpenTelemetry versions
        otlp_config = {
            "endpoint": otel_endpoint,
            "insecure": True
        }
        
        # Add compression only if supported by current OTel version
        # Some versions expect Compression enum, others expect string
        try:
            from opentelemetry.exporter.otlp.proto.grpc.trace_exporter import Compression
            otlp_config["compression"] = Compression.Gzip
        except ImportError:
            # Fallback to string-based compression for older versions
            try:
                otlp_config["compression"] = "gzip"
            except Exception as comp_error:
                # Skip compression if unsupported to maintain reliability
                logger.warning(f"OTLP compression not supported, continuing without compression: {comp_error}")
