...
class NakamaClient:
    ...
    async def _authenticate(self):
        ...
        try:
            ...
            # Use production-grade Nakama server key from environment
            nakama_server_key = os.getenv("NAKAMA_SERVER_KEY", "")
            if not nakama_server_key:
                logger.error("NAKAMA_SERVER_KEY environment variable not set")
                return
                
            # Create proper base64 encoded authorization header
            import base64
            auth_string = f"{nakama_server_key}:"
