...
class DatabaseService:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.pool = await asyncpg.create_pool(
                ...
                timeout=db_config.get('timeout', 60.0),
    ...
    async def _ensure_tables(self):
        ...
        async with self.pool.acquire() as conn:
            # Create maps table with error handling
            try:
                await conn.execute("""
                ...
                # Table creation conflicts can occur during rapid restarts
                logger.warning(f"Table creation warning (vps_maps): {e}")
                # Check if table exists
                result = await conn.fetchval("""
                    SELECT EXISTS (
                        SELECT FROM information_schema.tables 
                        WHERE table_name = 'vps_maps'
                    )
                """)
                if not result:
                    raise Exception(f"Failed to create vps_maps table: {e}")
